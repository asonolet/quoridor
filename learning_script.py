...
class DDDQNNet:
    def __init__(self, state_size, action_size, learning_rate, name):
        ...
        # We use tf.variable_scope here to know which network we're using (
        # DQN or target_net)
        # it will be useful when we will update our w- parameters (by copy
        # the DQN parameters)
        with tf.variable_scope(self.name):
            # We create the placeholders
            # *state_size means that we take each elements of state_size in
            ...
            """
            # Input is 100x120x4
            self.lay1 = tf.layers.dense(inputs=self.inputs_,
                                        units=512,
            ...
            ELU
            """
            self.lay2 = tf.layers.dense(inputs=self.lay1_out,
                                        units=512,
                                        activation=tf.nn.elu,
                                        kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                        name="layer2")
            ...
            ELU
            """
            self.lay3 = tf.layers.dense(inputs=self.lay2_out,
                                        units=512,
                                        activation=tf.nn.elu,
                                        kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                        name="layer3")
            ...
            self.value_fc = tf.layers.dense(inputs=self.lay3_out,
                                            units=512,
                                            activation=tf.nn.elu,
                                            kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                            name="value_fc")
            ...
            self.advantage_fc = tf.layers.dense(inputs=self.lay3_out,
                                                units=512,
                                                activation=tf.nn.elu,
                                                kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                                name="advantage_fc")
...
TargetNetwork = DDDQNNet(state_size, action_size, learning_rate,
                         name="TargetNetwork")

EnvNetwork = DDDQNNet(state_size, action_size, learning_rate,
                      name="EnvNetwork")
...
class SumTree:
    ...
    """
    Here we initialize the tree with all nodes = 0, and initialize the data with
    all values = 0
    """
    ...
    def __init__(self, capacity):
        ...
        # To understand this calculation (2 * capacity - 1) look at the
        # schema above
        # Remember we are in a binary node (each node has max 2 children) so
        # 2x size of leaf (capacity) - 1 (root node)
    ...
    """
    Here we add our priority score in the sumtree leaf and add the experience in
    data
    """
...
class Memory:  # stored as ( s, a, r, s_ ) in SumTree
    ...
    # have 0 probability of being taken
    PER_a = 0.6  # Hyperparameter that we use to make a tradeoff between
    ...
    def __init__(self, capacity):
        ...
        We prefer to use a simple array and to overwrite when the memory is
        full.
        """
    ...
    def sample(self, n):
        ...
        b_idx, b_ISWeights = np.empty((n,), dtype=np.int32), \
...
# MODIF actions --> possible_actions
...
# In our case we use it when we want to copy the parameters of DQN to
# Target_network
...
if training:
    with tf.Session() as sess:
        ...
        # TODO: ajouter une boucle sur les différents opposants, l'opposant
        # étant une copie du joueur précédent. Pour cela mettre à jour le
        # ``game`` qui prend comme arg le network adverse.
        ...
        for episode in range(total_episodes):
            ...
            # peuvent pas vraiment nous faire gagner (c'est trop tard!). Il
            # faudrait sortir de la boucle lorsque la partie est finie pour
            ...
            while step < max_steps:
                ...
                # With ϵ select a random action atat, otherwise select a =
                # argmaxQ(st,a)

                # TODO: get possible actions from game
                ...
                # TODO: du coup si j'ai bien compris la mémoire nous renvoie
                # en priorité les actions nouvelles qui n'ont jamais étées
                # vues par le réseau, et après je ne sais pas trop. Donc si on
                ...
                # DOUBLE DQN Logic
                # Use DQNNetwork to select the action to take at next_state (
                # a') (action with the highest Q-value)
                ...
                for i in range(0, len(batch)):
                    ...
                    if terminal:
                        ...
                        target = rewards_mb[i] + gamma * q_target_next_state[i][
                            action]
                ...
                if tau > max_tau:
                    # Update the parameters of our TargetNetwork with
                    # DQN_weights
                    update_target = update_target_graph()
                    sess.run(update_target)
                    tau = 0
